...
export const onLoadStart = async (url, authState, onSuccess, onError, close, getAccessToken, shouldGetAccessToken) => {
    console.log('URL in main function body'+JSON.stringify(url))
    if (isErrorUrl(url)) {
        console.log('URL in if condition'+JSON.stringify(url))
        const err = getErrorFromUrl(url);
        close();
        onError(transformError(err));
    }
    else {
        console.log('URL in else condition'+JSON.stringify(url))
...
export default class LinkedInModal extends React.Component {
    constructor() {
        ...
        this.onNavigationStateChange = async ({ url }) => {
            console.log('This is the url '+JSON.stringify(url))
            const { raceCondition } = this.state;
            const { redirectUri, onError, shouldGetAccessToken } = this.props;
            console.log('Redirect Url from props'+JSON.stringify(redirectUri))
            console.log('race condition state'+JSON.stringify(raceCondition))
